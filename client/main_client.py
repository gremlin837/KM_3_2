...
Скопировано и адаптировано из main_v2.py.   Admin@12345  itnoMPEI12345!   Popa12!
...
from audit.audit_service import AuditSystemFactory, EventType
    ...
    def login(self, username: str, password: str) -> tuple[bool, str, bool]:
        try:
            ...
            if r.status_code == 200:
                data = r.json()
                self.token = data["access_token"]  # ✅ Сохраняем токен сразу
                self.username = data["username"]
                self.is_admin = data["is_admin"]
                need_change = data.get("need_change", False)
                return True, data.get("detail", "Успешный вход"), need_change
            else:
...
class LoginDialog(QDialog):
    ...
    def _try_login(self):
        ...
        # ✅ Токен уже сохранён в api.token. Если нужна смена — вызываем диалог
        if need_change:
            ...
            if dlg.exec() == QDialog.DialogCode.Accepted:
                ...
                # Теперь запрос пройдёт, так как api.token установлен
                ...
                if chg_ok:
                    ...
                    self.accept()  # ✅ Закрываем окно входа, переходим в MainWindow
                    return
                else:
                    self.lbl_status.setText(chg_msg)
            else:
                self.lbl_status.setText("Смена пароля отменена.")

            self.btn_login.setEnabled(True)
            self.btn_login.setText("Войти")
            return

        # ✅ Обычный вход (без смены пароля)
        _audit_service.log_auth_login(u, "localhost", "PyQt6")
        self.accept()
...
class MainWindow(QMainWindow):
    ...
    def closeEvent(self, event):
        if not self._should_restart:
            # Закрытие через крестик (X) -> полный выход из приложения
            duration = int(time.time()) - self._login_time
            _audit_service.log_auth_logout(api.username, duration)
            sys.exit(0)
        event.accept()
