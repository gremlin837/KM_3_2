    ...
    def _try(self):
        ...
        if ok:
            ...
            if chg.exec() == QDialog.DialogCode.Accepted:
                self.logged_account = acc
                # Аудит: успешный вход после смены пароля
                self.audit_service.log_auth_login(u, "127.0.0.1", "PyQt6")
...
class MainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        # === ВКЛАДКА 4: АУДИТ (реальная логика из audit.py) ===
        t4 = QWidget()
        l4 = QVBoxLayout(t4)
    ...
    def _setup_signals(self):
        ...
        # При переключении на вкладку аудита — обновляем
    ...
    def _audit_refresh(self):
        """Загрузить события из БД и отобразить в таблице"""
    ...
    def _audit_send(self):
        """Отправка журнала на удалённый сервер (выбор папки)"""
    ...
    def _audit_rotate(self):
        """Ротация журналов: удаление старых записей с архивацией"""
        ...
        if result:
            QMessageBox.information(
                self, "Ротация",
                f"Ротация выполнена. Записи старше {days} дней архивированы и удалены."
            )
            self.audit_service.log_admin_params_change(
    ...
    # ------------------------------------------------------------------
    # Остальные методы (без и��менений)
    # ------------------------------------------------------------------
    ...
    def _calc(self):
        ...
        # Аудит: ввод данных
        self.audit_service.log_interface_input(
            self.acc.username, "calc_params", inp
        )
        self.pb.show()
        self.pb.setValue(0)
    ...
    def _vac(self):
        ...
        # Аудит: вывод результата вакуума
        self.audit_service.log_interface_output(
...
if __name__ == "__main__":
    ...
    # Создаём систему аудита через фабрику
